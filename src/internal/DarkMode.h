...
bool RefreshTitleBarThemeColor(HWND hWnd)
{
    ...
    if (g_buildNumber < 18362)
        if (SetProp(hWnd, TEXT("UseImmersiveDarkModeColors"), reinterpret_cast<HANDLE>(static_cast<INT_PTR>(dark))))
            return true;
...
void InitDarkMode()
{
    ...
    if (HMODULE handle = GetModuleHandle(TEXT("ntdll.dll")))
        _RtlGetNtVersionNumbers = reinterpret_cast<fnRtlGetNtVersionNumbers>(GetProcAddress(handle, "RtlGetNtVersionNumbers"));
    if (!_RtlGetNtVersionNumbers)
        return;

    DWORD major, minor;
    _RtlGetNtVersionNumbers(&major, &minor, &g_buildNumber);
    g_buildNumber &= ~0xF0000000;

    if (major == 10 && minor == 0 && CheckBuildNumber(g_buildNumber))
    {
        HMODULE hUxtheme = LoadLibraryEx(TEXT("uxtheme.dll"), nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
        if (!hUxtheme)
            return;
        ...
        if (HMODULE handle = GetModuleHandle(TEXT("user32.dll")))
            _SetWindowCompositionAttribute = reinterpret_cast<fnSetWindowCompositionAttribute>(GetProcAddress(handle, "SetWindowCompositionAttribute"));
